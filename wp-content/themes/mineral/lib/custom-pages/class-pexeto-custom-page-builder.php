	...
	public function get_custom_page_list_template( $post, $custom_page, $prefix ) {
		...
		if ( $custom_page->preview_image!='none' ) {
			...
			foreach ($images as $key => $image) {
				...
				$images[$key] = $prefix.$image;  //prepend the prefix to each preview image key
			}
			$img_str = implode('|', $images);

			$html.= '<img src="'.pexeto_get_resized_image($img_url, 200, 160).'" data-input_field="'.$img_str.'"/>';
		...
		foreach ( $custom_page->fields as $field ) {
			...
			case 'select':
				...
				foreach ($field['options'] as $option ) {
					...
					$html.='<option value="'.$option['id'].'"'.$data_hide.'>'.$option['name'].'</option>';
				}
				$html.='</select>';
				break;
			case 'colorpick':
