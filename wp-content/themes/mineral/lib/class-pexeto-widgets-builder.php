...
class PexetoWidgetsBuilder {
	...
	protected function print_after_field( $field ) {
		...
		if ( isset( $field['desc'] ) ) {
			...
			echo '<span class="help-button icon-help" aria-hidden="true" title="'.$field['name'].'"><div class="dialog-content"><p>'.$field['desc'].'</p></div></span>';
		}
	...
	public function print_upload( $field, $saved_val ) {
		...
		echo '<a id="'.$field['id'].'_button" class="upload-button pexeto-upload-btn"><span><i aria-hidden="true" class="icon-upload"></i>Upload</span></a></div>';
